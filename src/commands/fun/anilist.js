...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "user-search") {
      ...
      const userData = await fetchAniListData(userQuery, {
        userName: username,
      });
      ...
      const userEmbed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "Anime Watched",
            value: `${userData.User.statistics.anime.count}`,
            inline: true,
          },
          {
            name: "Episodes Watched",
            value: `${userData.User.statistics.anime.episodesWatched}`,
            inline: true,
          },
          {
            name: "Minutes Watched",
            value: `${userData.User.statistics.anime.minutesWatched}`,
            inline: true,
          },
          {
            name: "Manga Read",
            value: `${userData.User.statistics.manga.count}`,
            inline: true,
          },
          {
            name: "Volumes Read",
            value: `${userData.User.statistics.manga.volumesRead}`,
            inline: true,
          },
          {
            name: "Chapters Read",
            value: `${userData.User.statistics.manga.chaptersRead}`,
            inline: true,
          },
        );
      ...
      const title =
        animeData.Media.title.english || animeName || "Title not available";
      const genres =
        animeData.Media.genres.length > 0
          ? animeData.Media.genres.join(", ")
          : "No genres available";

      const animeEmbed = new EmbedBuilder()
        .setTitle(title)
        .setDescription(
          animeData.Media.description.replace(/<[^>]*>?/gm, "") ||
        ...
        .addFields(
          {
            name: "Episodes",
            value: `${animeData.Media.episodes || "No data available"}`,
            inline: true,
          },
